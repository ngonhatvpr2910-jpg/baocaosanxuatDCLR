const monthlyScrap = Array.from({length: 12}).map((_, i) => ({month: i+1, scrapCost: i < 6 ? 1000 : null}));
const weeklyScrap = [
...
const displayMonthlyScrap = monthlyScrap.map((m, i) => {
...
console.log(displayMonthlyScrap.filter(r => r.scrapCost !== null));
